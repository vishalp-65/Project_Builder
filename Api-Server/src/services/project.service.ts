...
class ProjectService {
    ...
    public static async createDeployment(data: {
        projectId: string;
        user: Required<
            Pick<User, "id" | "githubUsername" | "githubAccessToken">
        >;
        commitHash?: string;
    }) {
    ...
    related to repository access. */
        ...
        const environments = await this.getEnviromentVariables(
            projectId,
            user.id
        );
        ...
        let envData = environments.environmentVariables.map(
            (env: EnvVariables) => {
                return {
                    name: env.key,
                    value: env.value,
